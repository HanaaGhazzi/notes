...
describe('INPUT MODULE', () => {
    ...
    describe('getAction()', () => {
        ...
        it('asks to enter a valid action if an invalid action was entered', () => {
          const options = new Input();
          expect(options.getAction('Hanaa')).toEqual('Enter a valid action');
        });
    ...
    describe('valid()', () => {
        it('respects a proper object', () => {
          ...
          expect(options.getAction(options.action)).toBeTruthy() && 
          expect(options.getPayload(options.payload)).toBeTruthy(); 
